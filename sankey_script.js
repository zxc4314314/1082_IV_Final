    ...
    d3.csv("nodeWords.csv", function(error, nodeWordsData) {
        ...
        ******************/
        function showLinkWords(d) {
            console.log()
            var linkWordsDiv = document.getElementById('link_text_container').innerText = '流向中的情緒詞： ' + d.source.name + ' to ' + d.target.name + '\n' + '\n' + d.words+ '\n';
        }
        ...
        function showNodeWords(d, i) {
            ...
            for (var i = nodeWordsData.length - 1; i >= 0; i--) {
                if (nodeWordsData[i]["subset"] == d.name) {
                    var linkWordsDiv = document.getElementById('node_text_container').innerText = '節點鐘的情緒詞： ' + d.name + '\n' + '\n' + nodeWordsData[i]["words"] + '\n';
                }
            }
        }
    });
